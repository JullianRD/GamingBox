...
class ReviewRepository {
    ...
    static async findByIdComplete(id) {
        ...
        return rows[0] ? new Review(row[0]) : null;
    }
    ...
    static async update(id, data) {
        ...
        review_like = COALESCE($4, review_platine),
        ...
        game_platforme = COALESCE($8, gamePlatforme),
        updated_at = NOW()
        WHERE id_review = $9
        RETURNING
            id_review,
    ...
    static async delete(id) {
        const result = await db.query(
            /*SQL*/ `
            DELETE FROM reviews
            WHERE id_review = $1, [id]`
        );
        return result.rowCount > 0;
    }

    // Recherche d'une review par son titre, son slug ou nom de tag associé

    static async search(userId, term) {
        const query = /*sql*/ `
        SELECT *
        FROM v_reviews_search
        WHERE user_id = $1
        AND (
            review_title ILIKE $2
            OR slug ILIKE $2
            OR EXISTS (
                SELECT 1
